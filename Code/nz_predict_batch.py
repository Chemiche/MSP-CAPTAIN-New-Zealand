...
if use_STARt:
    ...
    reward_indx = 4
    result_dir = "predict_batch_nz_star_"
else:
    ...
    reward_indx = 5
    result_dir = "predict_batch_nz_last_epoch"
...
outfile = "predict_emp_" + str(SEED) + "_"
out_tag = ""
degrade_steps = 5
...
# save species table
sdm_pd = pd.DataFrame(np.nansum(sdms, axis=(1, 2)).astype(str).T, np.array(species_names).T)
sdm_pd.to_csv(os.path.join(data_wd, "species_tbl.csv"))
...
with open(summary_file, "w") as f:
    ...
    head = ["Seed", "Disturbance", "RestoredCells",
            ...
            "Mean_pr", "Min_pr", "Max_pr", "non_protected_sp",
...
for rep_i in range(SIMULATIONS):
    r_seeds_dict = get_r_seeds_dict(SEED * rep_i)
    ...
    envList = [copy.deepcopy(env_train)]
    ...
    n_nonprotected_species_total = np.sum(non_protected_species_id)

    protected_ranges = [np.mean(protected_range_fraction),
                        np.min(protected_range_fraction),
                        np.max(protected_range_fraction),
                        n_nonprotected_species_total] + list(protected_fr_risk_class)
    ...
    with open(summary_file, "a") as f:
        ...
        head = [SEED,
            ...
            m_init, p_init, t_init, r_init] + list(env_init.risk_label_counts()) + protected_ranges + [tot_reward]
    ...
    np.savez_compressed(
        file=os.path.join(results_wd, outfile + "stats_%s.npz" % rep_i),
...
np.save(os.path.join(results_wd, "protection_matrix.npy"), nzp)
print("Results saved in:", results_wd)

priority = np.einsum('rxy -> xy', nzp)

cn_util.plot_map(priority, z=reference_grid_pu_nan, nan_to_zero=False,
                 cmap="GnBu", show=show_plots, title="Selected areas (averaged)",
                 outfile=os.path.join(results_wd, "Protection_priorities.png"), dpi=350)




#
# f = "/Users/dsilvestro/Documents/Projects/Ongoing/Captain_NZ/batch_results/protection_matrix.npy"
# nzp = np.load(f)
# priority_ = np.einsum('rxy -> xy', nzp)
# priority = priority_ / np.max(priority_) * 100
#
# priority[priority < 20] = 20
# priority[priority > 80] = 100
# priority[np.where((priority > 20) & (priority < 80))] = 60
# priority[priority_ == 0] = 0
#
#
# cn_util.plot_map(priority, z=reference_grid_pu_nan, nan_to_zero=False, vmin=0, vmax=100,
#                  cmap="GnBu", show=False, title="Selected protection areas",
#                  outfile="/Users/dsilvestro/Documents/Projects/Ongoing/Captain_NZ/batch_results/Protection_priorities.png", dpi=350)
#
#
